  ...
  return (
    <div className="min-h-screen bg-gray-50 relative pb-24 overflow-hidden font-sans text-gray-900">
      <header className="bg-white px-6 py-4 border-b border-gray-200 flex items-center justify-between sticky top-0 z-50 shadow-sm">
        <div className="flex items-center gap-3"><Cpu className="w-8 h-8 text-vedam-blue" /><h2 className="text-xl md:text-2xl font-bold font-heading text-black">{title}</h2></div>
      ...
      <main className="container mx-auto px-6 lg:px-16 pt-24 relative z-10 text-center">
        ...
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-5xl md:text-7xl font-heading font-bold text-black mb-8">
          {title}
        </motion.h1>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3 }} className="bg-white p-12 rounded-3xl shadow-xl max-w-2xl mx-auto border border-gray-100">
          <div className="w-24 h-24 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-6"><Cpu className="w-12 h-12 text-vedam-blue animate-pulse" /></div>
          ...
          <a href="#contact" onClick={onBack} className="inline-block px-8 py-4 bg-black text-white font-bold rounded-xl shadow-lg hover:bg-gray-800 transition-colors">Request a Live Demo</a>
        </motion.div>

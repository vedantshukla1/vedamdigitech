...
const GenericSeoPage = ({ title, description, image, onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-white relative pb-24 overflow-hidden font-sans text-vedam-charcoal">
      <header className="bg-white px-6 py-4 border-b border-gray-200 flex items-center justify-between sticky top-0 z-50 shadow-sm">
        <div className="flex items-center gap-3"><TrendingUp className="w-8 h-8 text-vedam-gold" /><h2 className="text-xl md:text-2xl font-bold font-heading text-vedam-charcoal">{title}</h2></div>
      ...
      <main className="container mx-auto px-6 lg:px-16 pt-24 relative z-10 text-center">
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="max-w-4xl mx-auto mb-24 bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100">
          <div className="bg-vedam-cream border-b border-gray-100 p-6 flex justify-between items-center">
            <div>
          ...
          <div className="p-8 bg-white relative h-80 flex items-end justify-between gap-4">
            ...
            {[40, 65, 85, 120, 180, 260].map((h, i) => (
              <div key={i} className="w-full bg-vedam-charcoal/10 rounded-t-xl relative group hover:bg-vedam-charcoal/20 transition-colors" style={{ height: `${h}px` }}>
                <div className="absolute -top-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 font-bold text-sm bg-vedam-charcoal text-white px-2 py-1 rounded transition-opacity">{h * 12}</div>
              </div>
            ))}
          </div>
        </motion.div>
